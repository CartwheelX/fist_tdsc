...
# sssssssssssssssssss
    ...
    # def visualize_transformed_pvs_classwise(self, target_class):
        
    #     # load pertrub model first from the perturn model path
    ...
    #                 # Load batch data.
    #                 output, prediction, members, targets = pickle.load(f)
    #                 output, prediction, members, targets = (
    #                     output.to(self.device), prediction.to(self.device),
    #                     members.to(self.device), targets.to(self.device)
    #                 )
    ...
    #                 # Only attempt perturbation if both members and non-members exist.
    #                 if (members == 1).sum() > 0 and (members == 0).sum() > 0:
    ...
    #                     # ----- Overlap Detection via Cosine Similarity -----
    #                     # Compute cosine similarity between each non-member and every member.
    #                     cos_sim = F.cosine_similarity(
    #                         non_member_pvs.unsqueeze(1),   # shape: (n_non_members, 1, C)
    #                         member_pvs.unsqueeze(0),       # shape: (1, n_members, C)
    #                         dim=2
    #                     )
    #                     # For each non-member, take its maximum similarity with any member.
    #                     max_cos_sim, _ = cos_sim.max(dim=1)  # shape: (n_non_members,)
                        
    #                     cosine_threshold = self.cosine_threshold
    #                     alpha = 1

    #                     #  mask over non-members for those that exceed the threshold.
    #                     overlap_mask = max_cos_sim > cosine_threshold  # shape: (n_non_members,)

    #                     if overlap_mask.sum() > 0:
    #                         # Determine absolute indices (relative to the batch) for overlapping non-members.
    #                         overlapping_non_member_indices = non_member_indices[overlap_mask]
    ...
    #                             )
    #                         # Apply perturbation.
    #                         perturbed_output[selected_index] = output[selected_index] + alpha*learned_values
    #                         # Mark these non-members as perturbed.
    #                         batch_perturb_flags[selected_index] = True
    ...
    #         ax=ax[0]
    #     )
    #     ax[0].set_title("Before Perturbation", fontsize=14, fontweight='bold')
    ...
    #     # First, plot all perturbed points as before
    #     sns.scatterplot(
    ...
    #         ax=ax[1]
    #     )
    #     ax[1].set_title("After Perturbation", fontsize=14, fontweight='bold')
    #     ax[1].set_xlabel("t-SNE Component 1")
    #     ax[1].set_ylabel("t-SNE Component 2")
    #     ax[1].grid(True, linestyle="--", alpha=0.5)

    #     # Now, overlay a highlight for the perturbed non-members.
    #     # (Assuming non-members are those with membership_flags != 1)
    ...
    #         edgecolors='red',     # red outline for emphasis
    #         s=100,                # a larger marker size for highlighting
    #         linewidth=0.9,          # thicker edge line
    #         marker='o',
    #         label='Perturbed Non-Members'
    #     )

    #     # If you want a combined legend, you might need to merge legend handles.
    #     # For simplicity, here we add the new legend entry.
